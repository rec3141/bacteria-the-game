...
#
# Usage: ./deploy.sh                                  # publish to EVERY target below
#        ./deploy.sh --force                          # overwrite drifted files anyway (still backed up)
#        REMOTE_DIR=northinglab.com/bacteria ./deploy.sh   # just that one docroot
set -euo pipefail
cd "$(dirname "$0")"

REMOTE_HOST="${REMOTE_HOST:-dreamhost}"
# EVERY docroot this build publishes to, as "<home-relative dir>|<public url>". Home-relative so
# there is no local ~ expansion. Each target keeps its own scores.json, scenario queue and backup
# set, so the leaderboards stay separate and one deploy can never clobber the other's data.
#
# Deploying to both from one invocation is not a convenience, it is the point: publishing them
# separately meant two runs, two build stamps and two chances to forget, and northinglab sat 11 days
# and 40-odd commits behind cryomics without anything saying so. One run, one build, both sites.
TARGETS=(
  "bacteria.cryomics.org|https://bacteria.cryomics.org/"
  "northinglab.com/bacteria|https://northinglab.com/bacteria/"
)
# An explicit REMOTE_DIR narrows the run to that single docroot (SITE_URL follows, or is guessed).
if [ -n "${REMOTE_DIR:-}" ]; then
  TARGETS=("$REMOTE_DIR|${SITE_URL:-https://$REMOTE_DIR/}")
fi
...
REMOTE_HOME="$(ssh "$REMOTE_HOST" 'echo $HOME')"   # one host, so ask once
backup_slug() { printf '%s' "$1" | tr '/' '-'; }   # keep each target's backups apart
backup_dir()  { printf '%s/bacteria-backups/%s-%s' "$REMOTE_HOME" "$(backup_slug "$1")" "$STAMP"; }

echo "==> checking the live sites against this repo's history"
...
# Drifting files at ONE docroot, newline-separated on stdout.
drift_at() {
  local dir="$1" f rhash out=""
  for f in index.html game.js scores.php score_schema.php feedback.php scenario-request.php README.md manifest.webmanifest icon.svg Bacteria.swf .htaccess; do
    ...
    if is_stamped "$f"; then
      rhash="$(ssh "$REMOTE_HOST" "cat '$dir/$f' 2>/dev/null || true" | unstamp | git hash-object --stdin)"
    else
      rhash="$(ssh "$REMOTE_HOST" "cat '$dir/$f' 2>/dev/null || true" | git hash-object --stdin)"
    fi
    [ "$rhash" = "$(printf '' | git hash-object --stdin)" ] && continue  # absent remotely = a new file, not a conflict
    if ! known_blobs "$f" | sort -u | grep -qx "$rhash"; then out="$out$f
"; fi
  done
  printf '%s' "$out" | sed '/^$/d'
}

# EVERY target is checked BEFORE anything is written to any of them. Checking and deploying one at a
# time would let the first site publish and the second abort on drift, leaving the two on different
# builds — which is the exact condition this change exists to stop.
DRIFTED=0
for t in "${TARGETS[@]}"; do
  dir="${t%%|*}"
  printf '    %s ... ' "$dir"
  ...
  printf '           %s\n' $d
  echo "      inspect with:  ssh $REMOTE_HOST 'cat $dir/game.js' | diff - game.js"
done
...
if [ "$DRIFTED" -eq 1 ]; then
  ...
  echo
  if [ "$FORCE" -ne 1 ]; then
    printf "   Overwrite anyway? replaced files are backed up server-side  (type 'yes'): "
    read -r ans || ans=""   # EOF (piped / CI) counts as "no", not as a crash
    [ "$ans" = "yes" ] || { echo "==> aborted — nothing on any server was changed"; exit 1; }
  else
...
# ONE build id for the whole run, so both sites report the same stamp. Computing it per target
# would give them different ids for byte-identical code, and "which build is each site on?" is a
# question that has already cost us an afternoon.
BUILD="$(git rev-parse --short HEAD 2>/dev/null || echo dev)-$(date +%y%m%d%H%M)"
echo "==> build $BUILD → ${#TARGETS[@]} target(s)"
...
# Staged and stamped ONCE, then rsynced to each target: the two sites are guaranteed byte-identical
# rather than merely built from the same commit.
for t in "${TARGETS[@]}"; do
  REMOTE_DIR="${t%%|*}"; SITE_URL="${t##*|}"
  REMOTE="$REMOTE_HOST:$REMOTE_DIR/"
  BACKUP_SLUG="$(backup_slug "$REMOTE_DIR")"
  BACKUP_DIR="$(backup_dir "$REMOTE_DIR")"
  ...
  printf "      site  %s   %s\n" "$(curl -s -o /dev/null -w '%{http_code}' -L "$SITE_URL")" "$SITE_URL"
  printf "      api   %s   %sscores.php\n" "$(curl -s -o /dev/null -w '%{http_code}' -L "${SITE_URL}scores.php")" "$SITE_URL"
done
rm -rf "$STAGE"      # after the LAST target, not the first — every target rsyncs from this one copy

echo
echo "==> done — build $BUILD is live on:"
